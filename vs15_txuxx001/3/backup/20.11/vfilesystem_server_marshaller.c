...
int32_t marshall(uint8_t *data, uint32_t size, FileServerMessage *r_msg) {
	...
	switch(data[4]){
		...
		case FILE_INFO_RESPONSE:{
			FileInfoResponse* tmp = (FileInfoResponse*)msg-> payload;
			data[5] = tmp-> parent;
			data[6] = tmp-> parent >> 8;
		...
		case FOLDER_INFO_RESPONSE:{
			FolderInfoResponse* tmp = (FolderInfoResponse*)msg-> payload;
			data[5] = tmp-> parent;
			data[6] = tmp-> parent >> 8;
			data[7] = tmp-> parent >> 16;
			data[8] = tmp-> parent >> 24;
		...
		case READ_FILE_RESPONSE:{
			ReadFileResponse* tmp = (ReadFileResponse*)msg-> payload;
			data[5] = tmp-> size;
			data[6] = tmp-> size >> 8;
			data[7] = tmp-> size >> 16;
			data[8] = tmp-> size >> 24;
		...
		case ERROR_RESPONSE:{
			ErrorResponse* tmp = (ErrorResponse*)msg-> payload;
