...
FileID fs_new_file(char *name, FolderID parent){
	...
	if(folderFinder(parent, rootPointer, rfdv)){
		...
		File *newFile = (File *)malloc(sizeof(File)); 		
			...
			if(NULL == helper)	{
				rfdv-> destFolderPosition -> subFile = newFile;
...
int32_t deleteFolder(Folder *folder){
	...
	if(NULL != folder -> subFolder){								// delete all of folder in subfolder
	...
	if(NULL == folder -> subFile && NULL == folder -> subFolder){		// as no more files and folders,delete this folder
		printf("delete folderID : %d success\n", folder -> folderID);  
		if(folder == rootPointer){				
			free(folder -> folderName);		
			free(folder);
			rootPointer = NULL;											// if the deleted folder is root, set rootpointer null
		}
		else{
			free(folder -> folderName);		
			free(folder);
...
int32_t fs_delete_folder(FolderID folder) {
	...
	if(folderFinder(folder, rootPointer, rfdv)){	
		...
		if( rfdv ->destFolderPosition != rootPointer ){
				...
				if(rfdv -> parentPosition -> subFolder == rfdv -> destFolderPosition){		// to be deleted folder is the first folder
					flag =1;
