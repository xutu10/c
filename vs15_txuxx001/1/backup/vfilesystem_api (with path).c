...
int32_t fs_write_file(FileID file, uint32_t offset, uint32_t length, uint8_t *data){
	...
	//uint32_t fileSize = fs_get_file_size();??????
	uint8_t *tmpData = malloc((length + fileSize) * sizeof(uint8_t));
	uint32_t count = 0;
	if( NULL != get -> destFilePosition){
		uint8_t *helper = get -> destFilePosition -> data;
		while(count < offset){
			tmpData[count] = helper[count];
			count++;
		}
		while(count-offset+1 < length){
			tmpData[count] = data[count-offset+1];
			count++;
		}
		while(count < length + fileSize){
			tmpData[count] = helper[offset];
			count++;
			offset++;
		}
		free(helper);
		get -> destFilePosition -> data = tmpData;
		free(get-> path);
		free(get);
		return 0;	
	}
