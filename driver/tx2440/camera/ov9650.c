...
/* camera device(s) */
static struct tx2440_camif_dev camera;


/* image buffer for previewing. */
struct tx2440_camif_buffer img_buff[] =
{
	{
...
static void __inline__ camif_c2p(struct tx2440_camif_dev * pdev)
{
	/* 1. stop codec. */
	{
		u32 cicoscctrl;
...
static void __inline__ update_source_fmt_regs(struct tx2440_camif_dev * pdev)
{
	u32 cisrcfmt;
 ...
 *		CICOTAREA
 */
static void __inline__ update_target_fmt_regs(struct tx2440_camif_dev * pdev)
{
	u32 ciprtrgfmt;
	u32 ciprctrl;
	u32 ciprscctrl;
...
static void __inline__ update_target_wnd_regs(struct tx2440_camif_dev * pdev)
{
	u32 ciwdofst;
 ...
 *		CICOSCCTRL
 */
static void __inline__ update_target_zoom_regs(struct tx2440_camif_dev * pdev)
{
...
static void __inline__ update_camif_regs(struct tx2440_camif_dev * pdev)
{
	if (!in_irq())
	{
 ...
 */
static int start_capture(struct tx2440_camif_dev * pdev, int stream)
{
	int ret;
 ...
 *		CIIMGCPT
 */
static void stop_capture(struct tx2440_camif_dev * pdev)
{
	u32 ciprscctrl;
	u32 ciprctrl;
...
static void update_camif_config (struct tx2440_camif_fh * fh, u32 cmdcode)
{
	struct tx2440_camif_dev	* pdev;
...
static void init_camif_config(struct tx2440_camif_fh * fh)
{
	struct tx2440_camif_dev	* pdev;
...
static irqreturn_t on_camif_irq_c(int irq, void * dev)
{
	...
	pdev = (struct tx2440_camif_dev *)dev;
...
{
	s3c2410_gpio_cfgpin(S3C2410_GPG(11), S3C2410_GPIO_OUTPUT);
	s3c2410_gpio_setpin(S3C2410_GPG(11), 0);
	mdelay(20);
}
...
{
	s3c2410_gpio_cfgpin(S3C2410_GPG(11), S3C2410_GPIO_OUTPUT);
	s3c2410_gpio_setpin(S3C2410_GPG(11), 1);
	mdelay(20);
}
...
//	sccb_cleanup();
	...
	printk(KERN_ALERT"tx2440_camif: module removed\n");
}
